...
public class InitController {
    ...
    private void start(String imei) {
        ...
        //处理编辑继电器命令的信息
        relayDefinitionCommandRepository.save(new RelayDefinitionCommand(imei,"重置大棚指令","重置大棚指令",relays.get(1).getId()+"-0,"+relays.get(0).getId()+"-0",false,3000L,0L));
        ...
        relayDefinitionCommandRepository.save(new RelayDefinitionCommand(imei,"打开大棚指令","打开左右大棚",relays.get(1).getId()+"-1,"+relays.get(0).getId()+"-0",true,3000L,relayDefinitionCommandOptional.get().getId()));
        relayDefinitionCommandRepository.save(new RelayDefinitionCommand(imei,"关闭大棚指令","关闭左右大棚",relays.get(1).getId()+"-1,"+relays.get(0).getId()+"-1",true,3000L,relayDefinitionCommandOptional.get().getId()));
