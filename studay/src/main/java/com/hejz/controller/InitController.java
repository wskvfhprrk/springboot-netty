...
public class InitController {
    ...
    private void start(String imei, int i) {
        ...
        //处理编辑继电器命令的信息
        relayDefinitionCommandRepository.save(new RelayDefinitionCommand(imei, "重置大棚指令", "停止大棚电机指令", relays.get(1).getId() + "-0," + relays.get(0).getId() + "-0", false, 3000L, 0L, 0L));
        Optional<RelayDefinitionCommand> relayDefinitionCommandOptional = relayDefinitionCommandRepository.findByImei(imei).stream().filter(r -> r.getName().equals("重置大棚指令")).findFirst();
        relayDefinitionCommandRepository.save(new RelayDefinitionCommand(imei, "打开大棚指令", "打开左右大棚", relays.get(1).getId() + "-1," + relays.get(0).getId() + "-0", true, 3000L, relayDefinitionCommandOptional.get().getId(), i * 3 + 3L));
        relayDefinitionCommandRepository.save(new RelayDefinitionCommand(imei, "关闭大棚指令", "关闭左右大棚", relays.get(1).getId() + "-1," + relays.get(0).getId() + "-1", true, 3000L, relayDefinitionCommandOptional.get().getId(), i * 3 + 2L));
        if (!relayDefinitionCommandOptional.isPresent()) return;
        //处理感应器信息
        List<RelayDefinitionCommand> relayDefinitionCommands = relayDefinitionCommandRepository.findByImei(imei);
